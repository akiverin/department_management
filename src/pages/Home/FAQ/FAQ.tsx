...
function FAQ() {
	const questions = [
		...
		},
		{
			id: 3,
			question:
				'Какие сдавать экзамены, чтобы поступить на направление подготовки «Менеджмент»?',
			answer:
				'Какие сдавать экзамены, чтобы поступить на направление подготовки «Менеджмент»?'
		},
		{
		...
		<section className="faq">
			<div className="faq__wrapper">
				<div className="faq__info">
					...
					<h2 className="faq__title">Ответы на Ваши вопросы</h2>
					<p className="faq__desc">Остались вопросы? Свяжитесь с нами!</p>
					<div className="faq__detail">
						<img src={imageDetail} alt="" className="faq__image" />
						<h3 className="faq__detail-question">
							Как повысить свои шансы на поступление?
						</h3>
						<TheButton to="/" dark shadow>
							Узнать подробнее
						</TheButton>
					</div>
				</div>
				<ul className="faq__list">
					{questions.map(question => (
						<li className="faq__card" key={question.id}>
							<RowQuestion answer={question.answer}>
								{question.question}
							</RowQuestion>
						</li>
					))}
